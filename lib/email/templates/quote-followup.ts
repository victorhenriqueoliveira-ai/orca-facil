...
export function buildQuoteFollowupHtml(data: QuoteFollowupData): string {
  ...
  const { quote_number, quote_url, days_since_sent } = data;

  const daysText = days_since_sent === 1 ? "1 dia" : `${days_since_sent} dias`;
...
<head>
  ...
  <title>Follow-up: Orçamento #${quote_number} aguardando resposta — Orça Fácil</title>
</head>
          ...
          <tr>
            <td style="background-color:#C2703A;padding:10px 40px;text-align:center;">
              <p style="margin:0;color:#ffffff;font-size:13px;font-weight:600;letter-spacing:0.5px;text-transform:uppercase;">
                Orçamento aguardando resposta há ${daysText}
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:40px 40px 32px;">
              ...
              <p style="margin:0 0 20px;color:#4A3F38;font-size:15px;line-height:1.7;">
                ...
                <strong style="color:#2B2621;">${customerName}</strong> ainda não foi respondido.
                Faz <strong>${daysText}</strong> que ele está aguardando retorno — pode ser um bom momento para um follow-up!
              </p>
              ...
              <table cellpadding="0" cellspacing="0" role="presentation" style="width:100%;border:1px solid #E5DDD3;border-radius:10px;overflow:hidden;margin:28px 0;">
                <tr>
                  <td style="background-color:#FAF7F2;padding:20px 24px;">
                    <p style="margin:0 0 4px;color:#2D5D5A;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.6px;">Status do orçamento</p>
                    <p style="margin:0;color:#2B2621;font-size:15px;font-weight:500;">
                      Cliente: ${customerName}<br />
                      Orçamento: #${quote_number}<br />
                      Aguardando há: ${daysText}
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="background-color:#FAF7F2;border-top:1px solid #E5DDD3;padding:20px 40px;text-align:center;">
              <p style="margin:0 0 4px;color:#9B8E87;font-size:12px;">
                © Orça Fácil · Você está recebendo este e-mail porque um orçamento aguarda resposta.
              </p>
...
export async function sendQuoteFollowup(
  ...
  try {
    ...
    const customerName = data.customer_name ?? "Cliente";
    const daysText =
      data.days_since_sent === 1 ? "1 dia" : `${data.days_since_sent} dias`;

    const { data: resendData, error } = await resend.emails.send({
      from: "Orça Fácil <noreply@orcafacil.com.br>",
      to,
      subject: `${businessName}, o Orçamento #${data.quote_number} para ${customerName} aguarda resposta há ${daysText}`,
      html: buildQuoteFollowupHtml(data),
    });
