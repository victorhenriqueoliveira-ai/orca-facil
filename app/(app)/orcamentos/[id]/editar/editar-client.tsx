...
export default function EditarOrcamentoPage({
  ...
  const initialVersion = initialQuote?.versions?.[0];
  const [quote, setQuote] = useState<QuoteDetail | null>(initialQuote ?? null);
  const [rooms, setRooms] = useState<QuoteRoom[]>(initialVersion?.rooms ?? []);
  const [versionId, setVersionId] = useState<string | null>(initialVersion?.id ?? null);
  const [marginPct, setMarginPct] = useState(initialVersion?.profit_margin_pct ?? 0);
  const [marginInput, setMarginInput] = useState(String(initialVersion?.profit_margin_pct ?? 0));
  ...
  useEffect(() => { if (!initialQuote) fetchQuote(); }, [fetchQuote, initialQuote]);
