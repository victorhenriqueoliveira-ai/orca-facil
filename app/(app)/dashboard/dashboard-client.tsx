...
import type { ConversionMetrics } from "@/lib/dashboard/get-conversion";
...
interface DashboardClientProps {
  ...
  initialConversion: ConversionMetrics | null;
}

export function DashboardClient({ initialStats, initialConversion }: DashboardClientProps) {
  ...
  const defaultPeriodStart = getMesAtualInicio();
  const defaultPeriodEnd = getMesAtualFim();
  const [periodStart, setPeriodStart] = useState(defaultPeriodStart);
  const [periodEnd, setPeriodEnd] = useState(defaultPeriodEnd);
  const [conversion, setConversion] = useState<ConversionMetrics | null>(initialConversion);
  const [loadingConversion, setLoadingConversion] = useState(false);
  const [showGuia, setShowGuia] = useState(false);

  useEffect(() => {
    if (periodStart === defaultPeriodStart && periodEnd === defaultPeriodEnd) return;
